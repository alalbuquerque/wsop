...
=======
  ...
  <div class="mosca active"><div class="close"></div><span class="position"><marquee><a href="#">Inscreva-se já</a></marquee></span></div>
  <?php
  $antes = strtotime('05-10-2016 00:00:00'); // note que usei hífen
  $depois = strtotime(date("d-m-Y H:i:s")); // note que usei hífen
  $diferenca = ($depois - $antes);
  $valor_inicial = 300000000 + $diferenca;
  ?>
  <script type="text/javascript">
    setInterval(conta, 50);
    function conta(){
      if(document.getElementById('segundos').innerHTML == "")
        document.getElementById('segundos').innerHTML = "<?php echo $valor_inicial;?>";
      var valor_atual = document.getElementById('segundos').innerHTML;
      valor_atual = valor_atual.replace(",", "").replace(".", "").replace(".", "").replace(".", "");
      listValues =  valor_atual.split('');
...
/*
     ...
     if(curOffset >= 1100) {
>>>>>>> 093a2499920802461595d141b8a0294cbfb17493
